...
def test(test_loader, model, cfg, model_name):
    cfg.test_cfg.result_path = os.path.join(cfg.test_cfg.result_path, model_name)
    print('Saving to ', cfg.test_cfg.result_path)
    model.eval()
    ...
    if cfg.vis:
        vis = Visualizer(vis_path=osp.join(cfg.test_cfg.result_path, 'vis/'))
...
def main(args):
    ...
    test(test_loader, model, cfg, model_name)
